<template>
  <ion-page>
      ...
      <ion-toolbar color="bg-red-500">
        <ion-buttons slot="start">
          <div v-if="route.fullPath !='/home'" class="cursor-pointer" @click="router.go(-1)">
            <svg class="text-white w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
    ...
    <footer class="fixed bottom-0 left-0 right-0 bg-green-500 flex justify-between items-center py-4 px-6">
      <button @click="router.push('/home')" class="text-white focus:outline-none">
        <svg xmlns="http://www.w3.org/2000/svg" class="min-w-10 min-h-10" :class="{ 'text-green-700': route.fullPath === '/home' }" viewBox="0 0 24 24">
          <path fill="currentColor" d="M4 21V9l8-6l8 6v12h-6v-7h-4v7z" />
        </svg>
      </button>

      <button @click="router.push({name:'profile'})" class="text-white focus:outline-none">
        <svg :class="{ 'text-green-700': route.fullPath === '/home/profile' }" xmlns="http://www.w3.org/2000/svg" class="min-w-10 min-h-10" viewBox="0 0 24 24">
