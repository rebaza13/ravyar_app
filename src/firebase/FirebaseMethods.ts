...
export const deleteDocumentByQuery = async (collectionName: string, queryField: string, queryValue: string) => {
  try {
    const q = query(collection(db, collectionName), where(queryField, "==", queryValue));
    const querySnapshot = await getDocs(q);

    if (!querySnapshot.empty) {
      // Loop through each document matching the query
      querySnapshot.forEach(async (doc) => {
        await deleteDoc(doc.ref); // Delete each document with the matching query
        console.log(`Document in ${collectionName} with ID: ${doc.id} deleted`);
      });
    } else {
      console.log("No documents found for deletion based on the query.");
    }
  } catch (error) {
